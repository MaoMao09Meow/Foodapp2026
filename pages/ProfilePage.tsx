...
import { Edit3, Key, Star, MessageCircle, MapPin, X, Upload, Camera, Copy, Check } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-b from-blue-500 to-blue-600 p-8 pb-20 text-center relative">
        <div className="flex flex-col items-center">
          ...
          <div className="mt-2 flex items-center gap-2 bg-black/20 px-3 py-1 rounded-full text-white/90 text-[10px] cursor-pointer" onClick={copyUid}>
            <span>UID: {user.id}</span>
            {copied ? <Check size={10} className="text-green-300" /> : <Copy size={10} />}
          </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 space-y-4 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => setShowEditModal(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition">ยกเลิก</button>
              <button onClick={handleUpdateProfile} className="flex-1 py-3 bg-blue-500 text-white rounded-xl font-bold shadow-lg shadow-blue-100 active:scale-95 transition">บันทึก</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-3">
              <input type="password" placeholder="รหัสผ่านเดิม" className="w-full p-3 border rounded-xl bg-white text-black shadow-sm" value={oldPass} onChange={e => setOldPass(e.target.value)} />
              <input type="password" placeholder="รหัสผ่านใหม่" className="w-full p-3 border rounded-xl bg-white text-black shadow-sm" value={newPass} onChange={e => setNewPass(e.target.value)} />
              <input type="password" placeholder="ยืนยันรหัสผ่านใหม่" className="w-full p-3 border rounded-xl bg-white text-black shadow-sm" value={confirmPass} onChange={e => setConfirmPass(e.target.value)} />
            </div>
